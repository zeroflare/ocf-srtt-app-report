...
|------|------|------|------|------|------|------|
| `www.npa.gov.tw` | 內政部警政署 | 台灣 | 否（HiNet CDN） | AS3462（中華電信 HiNet） | | 警政署官網、各功能 WebView 內容頁 |
| `www.apb.npa.gov.tw` | 警政署刑事警察局 | 台灣 | 否（HiNet CDN） | AS3462（中華電信 HiNet） | | 刑事局相關查詢／內容頁 |
| `165.npa.gov.tw` | 刑事警察局 165 | 台灣 | 否（HiNet CDN） | AS3462（中華電信 HiNet） | | 165 反詐騙服務頁 |
| `adr.npa.gov.tw` | 警政署（防空避難） | 台灣 | 否（HiNet CDN） | AS3462（中華電信 HiNet） | | 防空避難處所查詢（`adr-web` 避難地圖／資訊） |
| `app110.npa.gov.tw` | 警政署（110 報案，GSN） | 台灣 | 否（政府網路） | AS4782（GSN） | | **110 定位報案 API**（`Service.asmx`，送出手機＋GPS） |
| `nv2.npa.gov.tw` | 警政署（GSN） | 台灣 | 否（政府網路） | AS4782（GSN） | | 警政後端服務（遺失物等） |
| `ps.npa.gov.tw` | 警政署（GSN） | 台灣 | 否（政府網路） | AS4782（GSN） | | 違規拖吊查詢（`TowingService`） |
| `eze8.npa.gov.tw` | 警政署（新世紀資通） | 台灣 | 否（政府網路） | AS9919（新世紀資通） | | 失竊車輛／通緝犯查詢（`NpaE8ServerRWD`） |
| `tm2.npa.gov.tw` | 警政署（新世紀資通） | 台灣 | 否（政府網路） | AS9919（新世紀資通） | | 交通違規／事故子系統 |
| `op2.npa.gov.tw` | 警政署（新世紀資通） | 台灣 | 否（政府網路） | AS9919（新世紀資通） | | 警政後端服務 |
| `eli.npa.gov.tw` | 警政署（新世紀資通） | 台灣 | 否（政府網路） | AS9919（新世紀資通） | | 警政後端服務 |
| `wmts.nlsc.gov.tw` | 內政部國土測繪中心（國網中心） | 台灣 | 否（政府機房） | AS7539（國網中心） | | 地圖圖磚（路況／測速點／避難底圖） |
| `vt.nlsc.gov.tw` | 內政部國土測繪中心（國網中心） | 台灣 | 否（政府機房） | AS7539（國網中心） | | 向量圖磚（避難處所點位 MVT） |
| `rtr.pbs.gov.tw` | 警察廣播電臺（GSN） | 台灣 | 否（政府網路） | AS4782（GSN） | | 收聽警廣（線上串流） |
| `165dashboard.tw` | 刑事局 165 打詐儀錶板（委外） | 日本 | **是（AWS EC2 東京）** | AS16509（AWS EC2） | | 打詐儀錶板前端與 `CIB_DWS_API` 統計 API |
| `api-next.no8.io` | 打詐儀錶板線上客服 API（委外 NO8） | 日本 | **是（AWS EC2 東京）** | AS16509（AWS EC2） | | 打詐儀錶板 `live_chat` API |
| `assets.no8.io` | 打詐儀錶板靜態資源（委外 NO8） | 台灣 | 是（AWS CloudFront） | AS16509（AWS） | ✓ | 靜態資源 |
| `live-chat-console.no8.io` | 打詐儀錶板線上客服（委外 NO8） | 台灣 | 是（AWS CloudFront） | AS16509（AWS） | ✓ | 線上客服／聊天 |
| `www/graph/web/api.facebook.com` 等 Meta 網域 | Meta | 台灣／美國 | 是 | AS32934（Meta） | ✓ | NPA署長室外連、FB SDK |
| `scontent.ftpe7-*.fna.fbcdn.net` 等 | Meta（FB CDN） | 台灣／美國 | 是 | AS32934（Meta） | ✓ | FB 圖片／靜態內容 |
| `cse.google.com`／`syndicatedsearch.goog` | Google | 台灣 | 是 | AS15169（Google） | ✓ | 站內搜尋（Custom Search） |
| `www.adsensecustomsearchads.com`／`*.adtrafficquality.google` | Google | 台灣 | 是 | AS15169（Google） | ✓ | 站內搜尋隨附廣告（AdSense for Search） |
| `www.googletagmanager.com`／`analytics.google.com`／`app-analytics-services.com` | Google | 台灣 | 是 | AS15169（Google） | ✓ | 使用行為分析（GA／GTM／Firebase） |
| `fonts.googleapis.com`／`fonts.gstatic.com` | Google | 台灣 | 是 | AS15169（Google） | ✓ | 網頁字型 |
| `i.ytimg.com`／`maps.googleapis.com` | Google | 台灣 | 是 | AS15169（Google） | ✓ | YouTube 影片縮圖／地圖 API |
| `unpkg.com` | Cloudflare | 台灣 | 是 | AS13335（Cloudflare） | ✓ | 前端 JS 函式庫 CDN |

> **國家判定說明（以本次實際連線 IP 為準）**：`.npa.gov.tw` 之 GSN 子網域（`app110`、`nv2`、`ps`，連 `210.69.154.x`，AS4782）與新世紀資通子網域（`eze8`、`tm2`、`op2`，連 `122.146.27.x`，AS9919）皆為政府／承包商實體機房，註冊國台灣，資料留在境內。`www.npa.gov.tw`、`www.apb.npa.gov.tw`、`165.npa.gov.tw`、`adr.npa.gov.tw` 由**中華電信 HiNet CDN（AS3462，`203.66.3x.x`）**承載，仍屬台灣電信基礎設施；`wmts`／`vt.nlsc.gov.tw`（`140.110.x.x`，AS7539 國網中心）亦為台灣。Google、Cloudflare 採 Anycast，連線節點多判定位於台灣，但**營運商為海外雲端**，資料出境記「可能」。Meta 同為 Anycast，**`*.facebook.com`／`connect.facebook.net` 等可回 `31.13.87.x`（`tpe1`，台灣；如 `www.facebook.com`→`31.13.87.36`）或 `57.144.4.x`（`lax3`，美國；如 `57.144.4.1`）**。**165 打詐儀錶板（`165dashboard.tw`、`api-next.no8.io`）經 mtr（TCP/443）與 PTR 反查確認為 AWS 東京 EC2（`ap-northeast-1`），為唯一資料實際落於境外之服務。**
...
| 網域 | 類型 | 連線 IP | ASN | 國家 |
|------|------|---------|-----|------|
| www.npa.gov.tw | A | 203.66.32.13 | AS3462（中華電信 HiNet） | 台灣 |
| www.apb.npa.gov.tw | A | 203.66.32.43 | AS3462（中華電信 HiNet） | 台灣 |
| 165.npa.gov.tw | A | 203.66.35.104 | AS3462（中華電信 HiNet） | 台灣 |
| adr.npa.gov.tw | A | 203.66.34.36 | AS3462（中華電信 HiNet） | 台灣 |
| app110.npa.gov.tw | A | 210.69.154.36 | AS4782（GSN） | 台灣 |
| nv2.npa.gov.tw | A | 210.69.154.93 | AS4782（GSN） | 台灣 |
| ps.npa.gov.tw | A | 210.69.154.38 | AS4782（GSN） | 台灣 |
| eze8.npa.gov.tw | A | 122.146.27.116 | AS9919（新世紀資通） | 台灣 |
| tm2.npa.gov.tw | A | 122.146.27.67 | AS9919（新世紀資通） | 台灣 |
| op2.npa.gov.tw | A | 122.146.27.88 | AS9919（新世紀資通） | 台灣 |
...
| 網域 | 類型 | 連線 IP | ASN | 國家 |
|------|------|---------|-----|------|
| wmts.nlsc.gov.tw | A | 140.110.20.85 | AS7539（國網中心） | 台灣 |
| vt.nlsc.gov.tw | A | 140.110.134.39 | AS7539（國網中心） | 台灣 |
| rtr.pbs.gov.tw | A | 117.56.47.51 | AS4782（GSN） | 台灣 |

AS7539 為 National Center for High-performance Computing（國網中心）；AS4782 為 GSN。

### 3. 165 打詐儀錶板（刑事局委外，AWS EC2 東京日本）
...
|------|------|---------|-------------------|-----|------|
| 165dashboard.tw | A | 52.197.64.104 | `ec2-52-197-64-104.ap-northeast-1.compute.amazonaws.com` | AS16509（AWS EC2） | 日本 |
| 165dashboard.tw | A | 54.95.60.138 | `ec2-54-95-60-138.ap-northeast-1.compute.amazonaws.com` | AS16509（AWS EC2） | 日本 |
| api-next.no8.io | A | 52.192.81.202 | `ec2-52-192-81-202.ap-northeast-1.compute.amazonaws.com` | AS16509（AWS EC2） | 日本 |
| api-next.no8.io | A | 13.113.30.63 | `ec2-13-113-30-63.ap-northeast-1.compute.amazonaws.com` | AS16509（AWS EC2） | 日本 |
| assets.no8.io | A | 65.9.180.10 | （CloudFront） | AS16509（AWS） | 台灣（邊緣） |
| live-chat-console.no8.io | A | 54.192.248.81 | （CloudFront） | AS16509（AWS） | 台灣（邊緣） |

AS16509 為 Amazon.com, Inc.（AWS）。PTR 主機名中的 `ap-northeast-1` 即東京區域，證實主機為該區 EC2 實例。

### 4. Facebook / Meta（NPA署長室外連與 FB SDK，海外）

* **域名性質**：`www/api/graph/web.facebook.com`、`rupload.facebook.com` 為 Meta 服務端點；`scontent.ftpe7-*.fna.fbcdn.net`、`scontent-tpe1-1.xx.fbcdn.net`、`static.xx.fbcdn.net` 為 FB 內容 CDN；`*-netseer-ipaddr-assoc.*.fbcdn.net` 為 Meta 網路測量
* **地理位置（mtr 443）**：ASN **AS32934（Meta）**，Anycast。**同一網域可視 DNS／選路落在台灣或美國邊緣**——`31.13.87.x`（PTR `…tpe1…`，約 5–28 ms，**台灣**；例：`www.facebook.com`→`31.13.87.36`）或 `57.144.4.x`（PTR `…lax3…`，約 145–195 ms，**美國洛杉磯**；例：`www.facebook.com`→`57.144.4.1`）。適用於 `www`／`graph`／`web`／`api`／`rupload.facebook.com`、`connect.facebook.net` 等。CDN：`scontent.ftpe7-*`→HiNet `203.74.65.17`（**台灣**）、`scontent-tpe1-1.xx.fbcdn.net`→`tpe1`（**台灣**），`static.xx.fbcdn.net` 亦可為 `tpe1`／`lax3`
* **基礎設施**：Meta 公有雲／邊緣
* **角色**：「NPA署長室」功能外連 Facebook 粉專；App 內嵌 FB SDK（登入／分享／內容嵌入）
* **資料特性**：傳送 App 識別、裝置資訊與 FB 互動資料；營運商為 Meta 海外（連線節點可為台灣或美國邊緣）
* **DNS／mtr 結果**（代表值；同一網域可回 tpe1 或 lax3）：

| 網域 | 連線 IP（代表） | PTR／節點 | 國家 |
|------|----------------|-----------|------|
| www.facebook.com | 31.13.87.36／57.144.4.1 | `…tpe1…`／`…lax3…` | 台灣／美國 |
| graph.facebook.com | 31.13.87.1／57.144.4.x | `…tpe1…`／`…lax3…` | 台灣／美國 |
| web.facebook.com | 31.13.87.x／57.144.4.141 | `…tpe1…`／`…lax3…` | 台灣／美國 |
| api.facebook.com | 31.13.87.x／57.144.4.141 | `…tpe1…`／`…lax3…` | 台灣／美國 |
| connect.facebook.net | 31.13.87.5／57.144.4.128 | `…tpe1…`／`…lax3…` | 台灣／美國 |
| rupload.facebook.com | 31.13.87.x／57.144.4.141 | `…tpe1…`／`…lax3…` | 台灣／美國 |
| scontent.ftpe7-1.fna.fbcdn.net | 203.74.65.17 | HiNet | 台灣 |
| scontent-tpe1-1.xx.fbcdn.net | 31.13.87.5 | `…tpe1…` | 台灣 |
| static.xx.fbcdn.net | 31.13.87.5／57.144.4.128 | `…tpe1…`／`…lax3…` | 台灣／美國 |

AS32934 為 Facebook, Inc.（Meta）。`tpe1`＝台北邊緣、`lax3`＝洛杉磯邊緣；**Meta 相關網域均可能在兩端之間切換**。
...
| 網域 | 類型 | 連線 IP | ASN | 國家 |
|------|------|---------|-----|------|
| cse.google.com | A | 142.250.192.142 | AS15169（Google） | 台灣 |
| www.googletagmanager.com | A | 142.250.77.200 | AS15169（Google） | 台灣 |
| ssl.google-analytics.com | A | 142.250.77.200 | AS15169（Google） | 台灣（Anycast） |
| fonts.gstatic.com | A | 142.250.77.195 | AS15169（Google） | 台灣（Anycast） |
| i.ytimg.com | A | 142.250.66.86 | AS15169（Google） | 台灣 |
...
「165 反詐騙」：`165.npa.gov.tw`（台灣，HiNet）承載反詐騙服務頁；「打詐儀錶板」載入 `165dashboard.tw/CIB_DWS_API`（AWS EC2 東京）之統計 API（`GetCounter`、`GetDailyCityFraudData`、`GetMonthlyFraudMethodRanking` 等，本次以 GET 讀取公開統計為主），並透過 `api-next.no8.io`（AWS EC2 東京）／`live-chat-console.no8.io`（AWS CloudFront）提供線上客服。**此類流量部分實際落於日本境外。**
...
使用者操作功能
  ├─ 110 定位報案 → 送出 手機＋GPS 個資 (app110.npa.gov.tw ← GSN, 台灣)
  ...
  ├─ 165 打詐儀錶板 → AWS EC2 東京 出境 (165dashboard.tw, api-next.no8.io)
...
| 165 打詐儀錶板 | `165dashboard.tw`、`api-next.no8.io` | 是（輔助） | **日本（AWS EC2 東京）** | **是** |
| 打詐儀錶板靜態／客服 | `assets.no8.io`、`live-chat-console.no8.io` | 否 | 台灣（AWS CloudFront） | 可能（AWS 海外營運） |
| Facebook / Meta | `*.facebook.com`、`fbcdn.net` | 否 | 台灣／美國（tpe1／lax3） | 可能（Meta 海外營運） |
...
值得注意的是，**「165 打詐儀錶板」（`165dashboard.tw`、`api-next.no8.io`）之前端與 API 主機本次實測位於 AWS EC2 東京（日本）**，是唯一資料明確落於境外之服務；其靜態資源與線上客服（`*.no8.io`）雖走 AWS 邊緣節點，營運商仍為海外雲端。此外，App 內多個 WebView 頁面嵌入 **Facebook 與 Google（站內搜尋／分析／字型／YouTube 縮圖）** 等第三方服務，連線節點雖多在台灣 Anycast／邊緣，營運商均為海外，會傳送瀏覽行為與裝置資訊，屬非核心之第三方流量。
