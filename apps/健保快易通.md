...
|------|------|------|------|------|------|------|
| `myhealthbank.nhi.gov.tw` | 健保署（GSN） | 台灣 | 否（政府網路） | AS4782（GSN） | | 健康存摺（就醫用藥／檢驗紀錄） |
| `mhbdata2.nhi.gov.tw` | 健保署（GSN） | 台灣 | 否（政府網路） | AS4782（GSN） | | 健康存摺資料 |
| `vhc.nhi.gov.tw` | 健保署（GSN） | 台灣 | 否（政府網路） | AS4782（GSN） | | 虛擬健保卡 |
| `med.nhi.gov.tw` | 健保署（GSN） | 台灣 | 否（政府網路） | AS4782（GSN） | | 醫療／就醫相關服務 |
| `info.nhi.gov.tw` | 健保署（GSN） | 台灣 | 否（政府網路） | AS4782（GSN） | | 健保資訊服務 |
| `cloudicweb.nhi.gov.tw` | 健保署（GSN） | 台灣 | 否（政府網路） | AS4782（GSN） | | 健保卡網路服務註冊 |
| `etask.nhi.gov.tw` | 健保署（GSN） | 台灣 | 否（政府網路） | AS4782（GSN） | | 健保署線上服務 |
| `www.nhi.gov.tw` | 健保署（公開網站） | 台灣 | **是（Cloudflare）** | AS13335（Cloudflare） | ✓ | 健保署官方網站（公開資訊） |
| `mnogw.cht.com.tw` | 中華電信（行動） | 台灣 | 否（電信機房） | AS17421（中華電信行動） | | 行動電話門號認證 |
| `login2.fetnet.net` | 遠傳電信 | 台灣 | 否（電信機房） | AS9674（遠傳電信） | | 行動電話門號認證 |
| `mobileconnect.gtcloud.net.tw` | 關貿網路（代管於數位聯合 Seednet） | 台灣 | 否 | AS7482（數位聯合 Seednet） | | 行動身分識別／門號認證 |
| `mid.twca.com.tw` | 臺灣網路認證 TWCA | 台灣 | 否（CA 機房） | AS9924（台灣固網） | | 行動身分識別 |
| `rootocsp.twca.com.tw` | 臺灣網路認證 TWCA | 台灣 | 否（CA 機房） | AS9924（台灣固網） / AS3462（中華電信 HiNet） | | 憑證狀態查詢（OCSP） |
| `twcasslocsp.twca.com.tw` | 臺灣網路認證 TWCA | 台灣 | 否（CA 機房） | AS9924（台灣固網） / AS3462（中華電信 HiNet） | | SSL 憑證狀態查詢（OCSP） |
| `wmts.nlsc.gov.tw` | 內政部國土測繪中心（國網中心） | 台灣 | 否（政府機房） | AS7539（國網中心） | | 就醫院所地圖圖磚 |
| `api.tgos.tw` | 內政部 TGOS 地圖服務 | 台灣 | **是（Google Cloud）** | AS396982（Google Cloud） | | 地圖／定位 API |
| `www.google-analytics.com` | Google | 台灣 | 是 | AS15169（Google） | ✓ | 使用行為分析 |
| `firebaselogging-pa.googleapis.com` | Google（Firebase） | 台灣 | 是 | AS15169（Google） | ✓ | Firebase 事件記錄 |
| `app-analytics-services.com` | Google | 台灣 | 是 | AS15169（Google） | ✓ | App 使用分析 |
| `www.googletagmanager.com` | Google | 台灣 | 是 | AS15169（Google） | ✓ | 代碼／追蹤管理 |
| `fcmtoken.googleapis.com` | Google（FCM） | 台灣 | 是 | AS15169（Google） | ✓ | 推播 Token |
| `device-provisioning.googleapis.com` | Google（FCM） | 台灣 | 是 | AS15169（Google） | ✓ | 推播裝置註冊 |
| `www.google.com` | Google | 台灣 | 是 | AS15169（Google） | ✓ | Google 一般服務 |
| `challenges.cloudflare.com` | Cloudflare（Turnstile） | 台灣 | 是（Cloudflare） | AS13335（Cloudflare） | ✓ | 公開網站人機驗證（bot 挑戰） |
| `www.jkos.com` | 街口網絡（MOBA） | 台灣 | 否 | AS131634（MOBA） | | 健保費線上繳費（街口支付選項） |

> **國家判定說明**：健保署所有 `*.nhi.gov.tw` 服務網域（含 `myhealthbank`、`mhbdata2`、`vhc`、`med`、`info`、`cloudicweb`、`etask`）解析至 `210.69.215.x`，ASN **AS4782（GSN，政府服務網路）**，為政府實體機房，資料留在台灣境內。身分認證鏈之網域經 DNS／traceroute 確認，均為**台灣境內**電信與憑證機構：中華電信行動（`mnogw.cht.com.tw`，AS17421）、遠傳（`login2.fetnet.net`，AS9674）、關貿網路（`mobileconnect.gtcloud.net.tw`，代管於數位聯合 Seednet AS7482）、臺灣網路認證 TWCA（`mid`／`rootocsp`／`twcasslocsp.twca.com.tw`，AS9924／AS3462）。`www.nhi.gov.tw`（公開網站）由 **Cloudflare（AS13335）** 承載，採 Anycast，連線節點判定位於台灣，但**營運商為海外雲端**，出境記「可能」。`api.tgos.tw`（地圖 API）由 **Google Cloud（AS396982）** 承載，連線節點在台灣，**非 Anycast**，惟營運商仍為海外雲端。Google 分析與推播服務（AS15169）同屬海外雲端。
...
* **地理位置**：`myhealthbank`／`mhbdata2`／`vhc`／`med`／`info.nhi.gov.tw` 解析至 `210.69.215.x`，ASN **AS4782（GSN，Data Communication Business Group）**，國家 **台灣**
...
| 網域 | 類型 | 解析 IP | ASN | 國家 |
|------|------|---------|-----|------|
| myhealthbank.nhi.gov.tw | A | 210.69.215.158 | AS4782（GSN） | 台灣 |
| mhbdata2.nhi.gov.tw | A | 210.69.215.149 | AS4782（GSN） | 台灣 |
| vhc.nhi.gov.tw | A | 210.69.215.143 | AS4782（GSN） | 台灣 |
| med.nhi.gov.tw | A | 210.69.215.170 | AS4782（GSN） | 台灣 |
| info.nhi.gov.tw | A | 210.69.215.152 | AS4782（GSN） | 台灣 |
| cloudicweb.nhi.gov.tw | A | 210.69.215.202 | AS4782（GSN） | 台灣 |
| etask.nhi.gov.tw | A | 210.69.215.166 | AS4782（GSN） | 台灣 |
...
| 網域 | 類型 | 解析 IP | ASN | 國家 |
|------|------|---------|-----|------|
| www.nhi.gov.tw | CNAME→A | 172.65.90.66 / 172.65.90.67 | AS13335（Cloudflare） | 台灣（Anycast；營運美國） |
...
| 網域 | 解析 IP | ASN | 所屬 | 國家 |
|------|---------|-----|------|------|
| mnogw.cht.com.tw | 211.79.35.179 | AS17421（中華電信行動） | 中華電信（行動） | 台灣 |
| login2.fetnet.net | 211.73.144.8 | AS9674（遠傳電信） | 遠傳電信 | 台灣 |
| mobileconnect.gtcloud.net.tw | 210.200.69.57 | AS7482（數位聯合 Seednet） | 關貿網路（代管數位聯合 Seednet） | 台灣 |
| mid.twca.com.tw | 219.87.64.184 / 60.250.3.152 | AS9924（台灣固網） / AS3462（中華電信 HiNet） | TWCA | 台灣 |
| rootocsp.twca.com.tw | 219.87.64.165 / 60.250.3.135 | AS9924（台灣固網） / AS3462（中華電信 HiNet） | TWCA | 台灣 |
| twcasslocsp.twca.com.tw | 219.87.64.165 / 60.250.3.135 | AS9924（台灣固網） / AS3462（中華電信 HiNet） | TWCA | 台灣 |
...
| 網域 | 類型 | 解析 IP | ASN | 國家 |
|------|------|---------|-----|------|
| wmts.nlsc.gov.tw | A | 140.110.134.19 | AS7539（國網中心） | 台灣 |
| api.tgos.tw | A | 34.160.0.116 | AS396982（Google Cloud） | 台灣 |
...
* **地理位置（本次實際連線 IP）**：`www.jkos.com` → `210.17.80.10`，ASN **AS131634（MOBA Private Limited Taiwan Branch）**，國家 **台灣**
* **基礎設施**：街口網絡自有 ASN，台灣境內
...
| 網域 | 連線 IP | ASN | 國家 |
|------|---------|-----|------|
| www.jkos.com | 210.17.80.10 | AS131634（MOBA Private Limited Taiwan Branch） | 台灣 |

AS131634 為 MOBA Private Limited Taiwan Branch（街口網絡台灣分公司）。
...
| 網域 | 類型 | 解析 IP | ASN | 國家 |
|------|------|---------|-----|------|
| www.google-analytics.com | A | 216.239.32.178 | AS15169（Google） | 台灣 |
| firebaselogging-pa.googleapis.com | A | 172.217.117.4 | AS15169（Google） | 台灣（Anycast） |
...
健保費線上繳費
  └─→ 街口支付選項 (www.jkos.com ← AS131634 MOBA)
```
...
| 健保費繳費（街口） | `www.jkos.com` | 否 | 台灣（AS131634 MOBA） | 否 |
