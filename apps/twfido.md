...
本報告依據 HAR 檔 `Stream-2026-07-14 15:07:17.har` 分析**行動自然人憑證**（`tw.gov.moi.tfido`）App 的網路請求。此 App 由內政部提供，主要用途為**身分驗證與數位簽章**，使用者可透過生物辨識完成免插卡、免帳密之政府服務身分驗證。

| 項目 | 內容 |
|------|------|
| App 套件名稱 | `tw.gov.moi.tfido` |
| App 版本 | 2.7.5（2606181） |
| 裝置 | iPhone 16 Plus / iOS 27.0 |
| 擷取時間 | 2026-07-14 15:03:16 ~ 15:03:51（UTC+8） |
| 總請求數 | 30 筆 |
| 涉及網域 | 3 個 |
...
| 網域 | 所屬單位 | 是否在台灣 | 是否為雲端服務 | 解析 IP | ASN | 國家 | 請求數 | 主要用途 |
|------|----------|------------|----------------|---------|-----|------|--------|----------|
| `fido.moi.gov.tw` | 內政部（MOI） | 是 | 否 | 117.56.7.69 | AS4782 | TW | 23 | 身分驗證、簽章、推播 Token 註冊、App 內容 |
| `iid.googleapis.com` | Google | 是（Anycast 節點） | 是（Firebase Cloud Messaging） | 見下方 DNS 解析 | AS15169 | TW | 4 | 推播通知 Token 註冊與 Topic 訂閱 |
| `app-analytics-services.com` | Google | 是（Anycast 節點） | 是（Firebase / Google Analytics） | 142.250.77.206 | AS15169 | TW | 3 | App 使用行為分析 |

> **國家判定說明**：除 `117.56.7.69`（GSN，WHOIS 確認）外，其餘 IP 依自台灣發起 ping 量測，**平均延遲 < 10ms 者判定連線節點位於台灣**。Google 服務採 Anycast，節點雖在台灣，營運商仍為 Google 海外雲端。
...
* **地理位置**：自台灣 ping 量測，所有解析 IP 延遲均 **< 10ms**，判定連線節點位於**台灣**（Google Anycast 邊緣節點）
...
| TTL | 類型 | 解析 IP | ASN | 國家 | Ping（ms） |
|-----|------|---------|-----|------|------------|
| 224 | A | 172.217.112.4 | AS15169 | TW | 6.9 |
| 224 | A | 172.217.113.4 | AS15169 | TW | 9.1 |
| 224 | A | 172.217.114.4 | AS15169 | TW | 5.2 |
| 224 | A | 172.217.115.4 | AS15169 | TW | 7.1 |
| 224 | A | 172.217.116.4 | AS15169 | TW | 5.5 |
| 224 | A | 172.217.117.4 | AS15169 | TW | 7.9 |
| 224 | A | 172.217.118.4 | AS15169 | TW | 7.0 |
| 224 | A | 172.217.119.4 | AS15169 | TW | 7.1 |
| 300 | A | 216.239.32.223 | AS15169 | TW | 5.9 |
| 300 | A | 216.239.34.223 | AS15169 | TW | 5.1 |
| 300 | A | 216.239.36.223 | AS15169 | TW | 6.6 |
| 300 | A | 216.239.38.223 | AS15169 | TW | 6.3 |

ASN **AS15169** 為 Google LLC。本次 HAR 實際連線 IP 為 `172.217.119.4`（ping 7.1ms，台灣節點）。

#### 3. app-analytics-services.com（Google，台灣 Anycast 節點，雲端）

* **域名性質**：Google Firebase Analytics / App Analytics 服務
* **地理位置**：IP `142.250.77.206`，ASN **AS15169**（Google LLC），ping 平均 **7.9ms**（< 10ms），判定連線節點位於**台灣**
...
| 方法 | 路徑 | 用途 | 次數 |
|------|------|------|------|
| GET | `/pt/news?IsApp=Y` | 最新消息列表 | 2 |
| GET | `/pt/news_detail?id=80085&IsApp=Y` | 消息詳情（例：v2.7.5 版本更新公告） | 1 |
| GET | `/pt/teaching?IsApp=Y` | 功能教學頁面 | 1 |
...
| 推播通知 | `iid.googleapis.com` | 是（輔助） | 台灣（Google Anycast，ping < 10ms） | 可能（Google 海外營運） |
| 使用分析 | `app-analytics-services.com` | 否 | 台灣（Google Anycast，ping < 10ms） | 可能（Google 海外營運） |

行動自然人憑證 App 的**核心身分驗證與簽章流程**完全走內政部自建之 `fido.moi.gov.tw`（台灣 GSN）。推播與分析雖連線至 Google 台灣 Anycast 節點（ping < 10ms），營運商仍為 Google 海外雲端，傳送內容為 Token 與使用事件，不含憑證私鑰或完整身分資料。
