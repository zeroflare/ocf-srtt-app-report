...
|------|------|------|------|------|------|------|
| `mfp.post.gov.tw` | 中華郵政 | 台灣 | 否（電信機房） | AS3462（中華電信 HiNet） / FET | | **App 自家事件遙測收集**（`/post/collect`） |
| `postserv.post.gov.tw` | 中華郵政 | 台灣 | 否（電信機房） | AS3462（中華電信 HiNet） / FET | | 郵務／i郵箱交易後端（`pstmail`） |
| `www.postmall.com.tw` | 中華郵政（郵政商城 ePost） | 台灣 | 否（電信機房） | FET | | 郵政商城／首頁工具列（`/epost/`） |
| `emap.post.gov.tw` | 中華郵政 | 台灣 | 否（電信機房） | AS3462（中華電信 HiNet） / FET | | 據點查詢地圖後端＋自有圖磚（`tu_m`） |
| `www.post.gov.tw` | 中華郵政 | 台灣 | 否（電信機房） | AS3462（中華電信 HiNet） | | 官方網站＋郵遞區號／地址查詢 |
| `mpost.post.gov.tw` | 中華郵政 | 台灣 | 否（電信機房） | FET / AS9924（台灣固網） | | 行動郵局服務（本次僅 CONNECT、未解密） |
| `mpush.post.gov.tw` | 中華郵政 | 台灣 | 否（電信機房） | AS7482（數位聯合 Seednet） / FET | | 推播通知（本次僅 CONNECT、未解密） |
| `ipost.post.gov.tw` | 中華郵政 | 台灣 | 否（電信機房） | AS9924（台灣固網） | | i 郵箱（本次未觸發，沿用初測） |
| `sslserver.twca.com.tw` | 臺灣網路認證 TWCA | 台灣 | 否（CA 機房） | AS9924（台灣固網） | | SSL 憑證服務 |
| `evsslocsp.twca.com.tw` | 臺灣網路認證 TWCA | 台灣 | 否（CA 機房） | AS9924（台灣固網） | | EV SSL 憑證狀態查詢（OCSP） |
| `rootocsp.twca.com.tw` | 臺灣網路認證 TWCA | 台灣 | 否（CA 機房） | AS9924（台灣固網） | | 根憑證狀態查詢（OCSP） |
| `tile.tracestrack.com` | Tracestrack（地圖圖磚） | 台灣 | 是（Cloudflare） | AS13335（Cloudflare） | ✓ | 據點查詢地圖圖磚 |
| `ssl.google-analytics.com` | Google | 台灣 | 是 | AS15169（Google） | ✓ | 使用行為分析 |
| `app-analytics-services.com` | Google | 台灣 | 是 | AS15169（Google） | ✓ | App 使用分析 |
| `fonts.gstatic.com` | Google | 台灣 | 是 | AS15169（Google） | ✓ | 網頁字型 |
...
| 網域 | 連線 IP | ASN／所屬 | 國家 |
|------|---------|-----------|------|
| mfp.post.gov.tw | 124.219.114/115.109、203.69.145.109 | FET／HiNet | 台灣 |
| postserv.post.gov.tw | 124.219.115.108 | FET | 台灣 |
| www.postmall.com.tw | 124.219.115.153 | FET | 台灣 |
| emap.post.gov.tw | 124.219.114.120 | FET | 台灣 |
| www.post.gov.tw | 175.98.228.167 | 台灣固網 | 台灣 |
| mpost.post.gov.tw | 175.98.228.114、124.219.115.114 | 台固／FET（僅 CONNECT） | 台灣 |
| mpush.post.gov.tw | 124.219.115.137 | FET（僅 CONNECT） | 台灣 |
...
| 網域 | 解析 IP | ASN | 國家 |
|------|---------|-----|------|
| sslserver.twca.com.tw | 219.87.64.186 | AS9924（台灣固網） | 台灣 |
| evsslocsp.twca.com.tw | 219.87.64.165 | AS9924（台灣固網） | 台灣 |
| rootocsp.twca.com.tw | 219.87.64.165 | AS9924（台灣固網） | 台灣 |

### 3. 地圖圖磚（Tracestrack，海外雲端）

* **域名性質**：`tile.tracestrack.com` 為 Tracestrack 地圖圖磚服務
* **地理位置（mtr 443）**：解析至 Cloudflare Anycast（`104.21.9.167`／`172.67.161.10`，AS13335），終點最佳延遲約 **5 ms** → **台灣**
* **角色**：「據點查詢」地圖底圖（搭配 OpenStreetMap 資料）；為功能性用途，開啟地圖時載入
* **資料特性**：地圖圖磚，不涉個資；連線節點在台灣，營運商為海外雲端（Cloudflare）
* **DNS／mtr 結果**：

| 網域 | 連線 IP | ASN | 國家 | Anycast |
|------|---------|-----|------|---------|
| tile.tracestrack.com | 104.21.9.167／172.67.161.10 | AS13335（Cloudflare） | 台灣 | ✓ |

### 4. Google 分析與字型（海外雲端）

* **域名性質**：`ssl.google-analytics.com`、`app-analytics-services.com`（使用行為分析）、`fonts.gstatic.com`（網頁字型）
* **地理位置（mtr 443）**：ASN **AS15169（Google LLC）**，Anycast；`ssl.google-analytics.com`／`fonts.gstatic.com` 終點約 **5–8 ms**（PTR `*.1e100.net`）→ **台灣**
* **角色**：收集 App 使用事件、載入網頁字型；屬非核心第三方輔助服務
* **資料特性**：傳送使用事件與裝置資訊；連線節點在台灣，營運商為 Google 海外
* **DNS／mtr 結果**：

| 網域 | 連線 IP | ASN | 國家 | Anycast |
|------|---------|-----|------|---------|
| ssl.google-analytics.com | 142.250.77.200 | AS15169（Google） | 台灣 | ✓ |
| app-analytics-services.com | 74.125.204.100 | AS15169（Google） | 台灣 | ✓ |
| fonts.gstatic.com | 142.250.77.195 | AS15169（Google） | 台灣 | ✓ |
...
App 冷啟動 / 郵務服務首頁
  ...
  └─→ 使用分析 / 字型 (Google，台灣 Anycast) (ssl.google-analytics.com, fonts.gstatic.com)
...
| 地圖圖磚（Tracestrack） | `tile.tracestrack.com` | 是（輔助） | 台灣（Cloudflare Anycast） | 可能 |
| Google 分析／字型 | `ssl.google-analytics.com`／`fonts.gstatic.com` 等 | 否 | 台灣（Anycast） | 可能 |

行動郵局的**核心郵務、郵政商城與（未測試之）儲匯服務**走中華郵政自有的 `*.post.gov.tw`／`postmall.com.tw`，託管於多家台灣電信業者（HiNet、遠傳/亞太、台固、Seednet），皆在台灣境內，此部分資料保護符合期待。值得一提的是，App 的**使用行為遙測以自建的 `mfp.post.gov.tw/post/collect`（台灣境內）收集**，相較將行為外送第三方之作法，隱私上較佳；另仍有一份 Google 使用分析與網頁字型（台灣 Anycast，營運海外）並存。「據點查詢」地圖底圖由 Tracestrack（Cloudflare）提供，屬非核心之第三方輔助。

> **隱私風險評估**：以可重現、可歸屬於行動郵局之流量而言，核心郵務、商城與遙測後端皆在台灣境內，風險低。複測確認查詢與遙測資料未出境；`mpost`／`mpush` 雖因憑證綁定未能解密，惟連線 IP 仍在台灣。境外營運之輔助流量包括地圖圖磚（Tracestrack）與 Google 分析／字型（連線節點為台灣 Anycast）。本次仍為未登入測試，登入與寄件、儲匯流程之個資傳輸內容，建議後續針對單一功能加測以完整評估。
